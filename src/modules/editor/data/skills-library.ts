...
/** A named bucket of skills. The library is organised into 9 sections
 *  that the picker uses as tree-rail rows. Order here is the order
 *  shown to the user. */
export interface SkillSection {
  /** Human-readable group label (e.g. "数据分析"). Used directly as the
   *  rail row + sticky list header. */
  name: string
  skills: SkillCard[]
}

export const SKILLS_BY_SECTION: SkillSection[] = [
  {
    name: '数据分析',
    skills: [
...
/** Flat view of every skill, in section + section-internal order. Kept
 *  for back-compat with consumers that don't care about grouping
 *  (e.g. the persona-store skill catalog lookup). */
export const SKILLS_LIBRARY: SkillCard[] = SKILLS_BY_SECTION.flatMap((s) => s.skills)
